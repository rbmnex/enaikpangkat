<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header"></div>
                  <div class="card-body">
                    <p>Assalamualaikum & Salam Sejahtera</p>

                  <p>Tuan/Puan,</p>

                  <p><strong>PEMBERITAHUAN PERTUKARAN {{ $jawatan }}</strong></p>

                  <p>Dengan segala hormatnya perkara di atas adalah dirujuk.</p>

                  <p>2.        Bersama-sama ini disertakan Surat Pemberitahuan Pertukaran yang menerangkan maksudnya untuk tindakan tuan/puan selanjutnya</p>

                  <p>3.        Sila ambil tindakan berikut :.</p>
                  <p>
                  <span>             i.   Sahkan penerimaan pemberitahuan ini melalui emel penempatan@jkr.gov.my atau telefon seperti di bawah.</span><br/>

                  <span>             ii.   Tuan/puan boleh menggunakan salinan surat yang dilampirkan untuk tujuan melapor diri.</span><br/>

                  <span>             iii.  Sila ambil surat asal di Tingkat 30 Blok G, Ibu Pejabat JKR Kuala Lumpur</span>
                  </p>
                  <p>Sekian, terima kasih.</p> 

                  <p><strong>`BERKHIDMAT UNTUK NEGARA’</strong></p>

                  <p>Saya yang menjalankan amanah,</p>


                  <p>
                    <span>KETUA BAHAGIAN PEMBANGUNAN SKIM KEJURUTERAAN</span><br/>
                    <span>Cawangan Dasar Dan Pengurusan Korporat</span><br/>
                    <span>Tingkat 30 Blok G (Bangunan KKR2),</span><br/>
                    <span>Ibu Pejabat JKR Malaysia</span><br/>
                    <span>No.6, Jalan Sultan Salahuddin</span><br/>
                    <span>50480 KUALA LUMPUR</span><br/>
                  </p>
                        <p>Pautan Link  :   {{ $link }}</p>
                        <p>Pautan Pink Form  :   {{ $pink }}</p>

                  </div>
               </div>
           </div>
       </div>
   </div>
</div>
